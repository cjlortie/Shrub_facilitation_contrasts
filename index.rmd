...
cacti.simp <- cacti %>% select(species, year, rii.richness)


#shrub-cacti contrast
shrubs.2017 <- foundation.rii %>% filter(year == 2017)
names <- sites %>% select(site.name, species)
  
shrubs.2017 <- left_join(shrubs.2017, names)

shrubs.2017 <- shrubs.2017 %>% rename("rii.richness" = "richness") %>%
  select(year, species, rii.richness)

foundationspecies <- dplyr::bind_rows(shrubs.2017, cacti.simp)
